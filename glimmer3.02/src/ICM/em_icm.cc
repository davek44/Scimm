...
void  em_ICM_t :: Build_Indep_WO_Stops
...
void  em_ICM_t :: Build_Reverse_Codon_WO_Stops
...
void  em_ICM_t :: Cumulative_Score
...
//**ALD
//printf ("Cumulative_Score:  i = %2d  ch = %c  prob = %6.4f\n", i, s [i], exp (x));
      result += x;
...
//**ALD
//printf ("Cumulative_Score:  %-.*s  start = %3d  i = %3d  ch = %c  prob = %6.4f\n",
//     model_len, cstr + start, start, i, s [i], exp (x));
      result += x;
      if  (frame == periodicity - 1)
          frame = 0;
        else
...
void  em_ICM_t :: Cumulative_Score_String
...
void  em_ICM_t :: Full_Window_Distrib
...
double  em_ICM_t :: Full_Window_Prob
...
double  em_ICM_t :: Partial_Window_Prob
...
void  em_ICM_t :: Set_Label_String
...
em_ICM_Training_t :: em_ICM_Training_t
    (int w, int d, int p)  :  ICM_t (w, d, p)

//  Constructor for the ICM

  {
   double  (* ptr) [ALPHA_SQUARED];
   int  i, j;

   train = (em_ICM_Training_Node_t * *)
                  Safe_calloc (periodicity, sizeof (em_ICM_Training_Node_t *),
                  __FILE__, __LINE__);

   if  (model_depth == 0)
       ptr = NULL;
     else
       ptr = count_memory = (double (*) [ALPHA_SQUARED])
               Safe_calloc (periodicity * num_nodes * (model_len - 1),
                            sizeof (double [ALPHA_SQUARED]), __FILE__, __LINE__);

   for  (i = 0;  i < periodicity;  i ++)
     {
      train [i] = (em_ICM_Training_Node_t *)
...
void  em_ICM_Training_t :: Complete_Tree
      ...
      for  (frame = 0;  frame < periodicity;  frame ++)
        {
         ...
         for  (sub = first_node;  sub <= last_node;
           ...
           {
            double  final_char_ct [ALPHABET_SIZE] = {0};
            ...
            max_pos = 0;
            best_info = em_Get_Mutual_Info (train [frame] [sub] . count [0],
                                         ALPHABET_SIZE, sum);
            used_info = best_info;

            for  (i = 1;  i < model_len - 1;  i ++)
              {
               next_info = em_Get_Mutual_Info (train [frame] [sub] . count [i],
                                            ALPHABET_SIZE, sum);
            ...
            if  (Verbose > 1)
                {
                 ...
                 for  (i = 0;  i < ALPHABET_SIZE;  i ++)
                   fprintf (stderr, " %4f", final_char_ct [i]);
                 fputc ('\n', stderr);
                }
...
void  em_ICM_Training_t :: Count_Char_Pairs_Restricted
...
//  pair at positions  j  and  (model_len - 1)  in the window.
  ...
  {
   em_ICM_Training_Node_t  * node;
   int  start, stop, end, frame, last_char_sub;
   int  i, j;

   start = 0;
   end = int (strlen (string . seq));
   frame = model_len % periodicity;

   for  (stop = model_len - 1;  stop < end;  start ++, stop ++)
     {
      node = Get_Training_Node (string . seq + start, frame, level);
      if  (node != NULL)
          {
           last_char_sub = Subscript (string . seq [stop]);
           for  (i = 0;  i < model_len - 1;  i ++)
             {
              j = ALPHABET_SIZE * Subscript (string . seq [start + i])
                      + last_char_sub;
              node -> count [i] [j] += string . p;
             }
          }
...
em_ICM_Training_Node_t *  em_ICM_Training_t :: Get_Training_Node
...
void  em_ICM_Training_t :: Interpolate_Probs
...
void  em_ICM_Training_t :: Take_Logs
    (void)
...
void  em_ICM_Training_t :: Train_Model
    (const vector <ICM_Training_Data_t> & data)
   ...
   for  (frame = 0;  frame < periodicity;  frame ++)
     {
      ...
      if  (model_depth == 0)
          {
           for  (i = 0;  i < string_ct;  i ++)
             em_Count_Single_Chars (final_char_ct, data [i] . seq + offset,
                                 data [i] . p, model_len, periodicity);
           sum = 0;
           for  (i = 0;  i < ALPHABET_SIZE;  i ++)
             sum += final_char_ct [i];
           for  (i = 0;  i < ALPHABET_SIZE;  i ++)
	    ...
	    // for each position in the window, count pairs of nt's with that position and the last position
           for  (i = 0;  i < string_ct;  i ++)
             em_Count_Char_Pairs (train [frame] [0] . count,
                               data [i] . seq + offset, data [i] . p, model_len, periodicity);
           ...
           // find the position pair with the max mutual information
           max_pos = 0;
           best_info = em_Get_Mutual_Info (train [frame] [0] . count [0],
                                        ALPHABET_SIZE, sum);

           for  (i = 1;  i < model_len - 1;  i ++)
             {
              next_info = em_Get_Mutual_Info (train [frame] [0] . count [i],
                                           ALPHABET_SIZE, sum);
              if  (next_info >= best_info)
                  {
                   best_info = next_info;
